...
fn word_found_in_set(
    ...
    for ocr_w in ocr_words {
        ...
        // Sliding window for MERGED tokens ("Sevagoth Prime" → "sevagotfirime").
        // ANCHOR at the start of the OCR token (allowing one leading stray glyph):
        // a card's base name leads its text, so in a merged token it sits at the
        // front. Scanning ALL positions let a short base name match as an interior
        // substring of a longer, DIFFERENT word — e.g. "gara" inside "akjagara"
        // wrongly matched "Gara Prime" on an Akjagara card.
        let ob = ocr_w.as_bytes();
        if ob.len() >= wb.len() {
            let max_start = 1.min(ob.len() - wb.len());
            for start in 0..=max_start {
                let win = &ob[start..start + wb.len()];
