...
export default function App() {
  ...
  const settingsRef = useRef({
    overlayEnabled: true, overlayPriority: "completion", textScale: 1, colorblindMode: false, companionApiEnabled: false, memoryScannerEnabled: false, blobLogEnabled: false, uiScale: 1,
  ...
  settingsRef.current = { overlayEnabled, overlayPriority, textScale, colorblindMode, companionApiEnabled, memoryScannerEnabled, blobLogEnabled, uiScale, tracked, favorites, timerFavorites, fissureWatches, modularWidth, modularSectionOrder, modularPopout };
  ...
  // Start/stop the monitor with the toggle (upstream's model). The monitor runs
  // the memory scan AND the relic reward OCR, so the relic overlay is active only
  // while the scanner is on. (Riven detection is on the always-on log watcher.)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    invoke<boolean>("get_monitor_status").then(active => {
      if (!active) {
        ...
        // the toggle effect above handles delayed auto-start after settings load.
      } else {
        setMonitoring(true);
      }
    });
  ...
  useEffect(() => { if (settingsLoadedRef.current) saveAllSettings(); }, [timerFavorites]);        // eslint-disable-line
  useEffect(() => { if (settingsLoadedRef.current) saveAllSettings(); }, [fissureWatches]);        // eslint-disable-line
  ...
  useEffect(() => { if (settingsLoadedRef.current) saveAllSettings(); }, [blobLogEnabled]);        // eslint-disable-line
